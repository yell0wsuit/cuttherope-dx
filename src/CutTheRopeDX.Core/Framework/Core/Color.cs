...
namespace CutTheRopeDX.Framework.Core
{
    ...
    public struct Color(byte r, byte g, byte b, byte a)
    {
        ...
        /// (<c>(byte)Math.Clamp(value * 255, byte.MinValue, byte.MaxValue)</c>) so colors
        ...
        public Color(float r, float g, float b, float a)
            : this(
                (byte)Math.Clamp(r * 255, byte.MinValue, byte.MaxValue),
                (byte)Math.Clamp(g * 255, byte.MinValue, byte.MaxValue),
                (byte)Math.Clamp(b * 255, byte.MinValue, byte.MaxValue),
                (byte)Math.Clamp(a * 255, byte.MinValue, byte.MaxValue))
        {
        }
