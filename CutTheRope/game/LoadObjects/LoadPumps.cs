...
namespace CutTheRope.game
{
    ...
    internal sealed partial class GameScene
    {
        ...
        /// </summary>
        private void LoadPump(XMLNode xmlNode, float scale, float offsetX, float offsetY, int mapOffsetX, int mapOffsetY)
        {
            ...
            pump.initial_x = pump.x = (xmlNode["x"].IntValue() * scale) + offsetX + mapOffsetX;
            pump.initial_y = pump.y = (xmlNode["y"].IntValue() * scale) + offsetY + mapOffsetY;
            pump.initial_rotation = 0f;
            pump.initial_rotatedCircle = null;
            pump.rotation = xmlNode["angle"].FloatValue() + 90f;
            pump.UpdateRotation();
            pump.anchor = 18;
